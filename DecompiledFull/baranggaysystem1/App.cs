...
public class App : Application
{
	...
	protected override void OnStartup(StartupEventArgs e)
	{
		...
		if (UserManualCaptureRunner.IsRequested(e.Args))
		{
			base.OnStartup(e);
			AppLogger.Initialize();
			((DispatcherObject)this).Dispatcher.BeginInvoke((Delegate)(Action)async delegate
			{
				int exitCode = await UserManualCaptureRunner.RunAsync(e.Args);
				Shutdown(exitCode);
	...
	private static void TryActivateRunningInstance()
	{
		try
		{
			...
			nint num = FindWindowHandle(process2);
			if (num != IntPtr.Zero)
			{
	...
	private static nint FindWindowHandle(Process process)
	{
		...
		EnumWindows(delegate(nint hWnd, nint _)
		{
			...
			if (processId == (uint)process.Id && IsWindowVisible(hWnd))
			{
				found = hWnd;
				return false;
			}
	...
	public void InitializeComponent()
	{
		...
		{
			_contentLoaded = true;
			Uri resourceLocator = new Uri("/baranggaysystem1;V3.0.0;component/app.xaml", UriKind.Relative);
