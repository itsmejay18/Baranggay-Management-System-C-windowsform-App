...
public class CollectionsPage : UserControl, IComponentConnector
{
	...
	internal TextBlock sectionBadgeText;

	internal Button btnAddRecord;

	internal TextBlock totalExpenseMetric;

	internal TextBlock pendingExpenseMetric;

	internal TextBlock inventoryValueMetric;

	internal TextBlock lowStockMetric;

	internal TextBlock activeAssetMetric;

	internal TextBox searchBox;

	internal ComboBox categoryFilter;

	internal ComboBox statusFilter;

	internal TabControl financeTabs;

	internal DataGrid expenseGrid;

	internal StackPanel expenseEmptyState;

	internal TextBlock expenseEmptyLabel;

	internal DataGrid inventoryGrid;

	internal StackPanel inventoryEmptyState;

	internal TextBlock inventoryEmptyLabel;

	internal DataGrid assetGrid;

	internal StackPanel assetEmptyState;

	internal TextBlock assetEmptyLabel;
	...
	internal Button btnEditSelected;

	internal Button btnViewSelected;

	internal TextBlock footerCountLabel;

	internal TextBlock footerHintLabel;
	...
	private async void BtnAdd_Click(object sender, RoutedEventArgs e)
	{
		...
		try
		{
			...
			case FinanceSection.Inventory:
			{
				...
				if (DialogService.Instance.ShowDialog(inventoryItemWindow) == true)
				{
			...
			case FinanceSection.Assets:
			{
				...
				if (DialogService.Instance.ShowDialog(assetRecordWindow) == true)
				{
			...
			default:
			{
				...
				if (DialogService.Instance.ShowDialog(expenseEntryWindow) == true)
				{
	...
	{
		return table?.AsEnumerable().Sum((DataRow row) => ReadDecimal(row, columnName)) ?? 0m;
	}

	private static decimal SumRows(DataTable? table, Func<DataRow, bool> predicate, string columnName)
	{
		return table?.AsEnumerable().Where(predicate).Sum((DataRow row) => ReadDecimal(row, columnName)) ?? 0m;
	}
	...
	public void InitializeComponent()
	{
		...
		{
			_contentLoaded = true;
			Uri resourceLocator = new Uri("/baranggaysystem1;V3.0.0;component/views/pages/collectionspage.xaml", UriKind.Relative);
	...
	void IComponentConnector.Connect(int connectionId, object target)
	{
		...
		case 1:
			...
			break;
		case 2:
			sectionBadgeText = (TextBlock)target;
			break;
		case 3:
			btnAddRecord = (Button)target;
			btnAddRecord.Click += BtnAdd_Click;
			break;
		case 4:
			totalExpenseMetric = (TextBlock)target;
			break;
		case 5:
			pendingExpenseMetric = (TextBlock)target;
			break;
		case 6:
			inventoryValueMetric = (TextBlock)target;
			break;
		case 7:
			lowStockMetric = (TextBlock)target;
			break;
		case 8:
			activeAssetMetric = (TextBlock)target;
			break;
		case 9:
			...
			break;
		case 10:
			categoryFilter = (ComboBox)target;
		...
		case 12:
			...
			break;
		case 13:
			financeTabs = (TabControl)target;
			financeTabs.SelectionChanged += FinanceTabs_SelectionChanged;
			break;
		case 14:
			expenseGrid = (DataGrid)target;
			expenseGrid.SelectionChanged += ExpenseGrid_SelectionChanged;
			break;
		case 15:
			expenseEmptyState = (StackPanel)target;
			break;
		case 16:
			expenseEmptyLabel = (TextBlock)target;
			break;
		case 17:
			inventoryGrid = (DataGrid)target;
			inventoryGrid.SelectionChanged += InventoryGrid_SelectionChanged;
			break;
		case 18:
			inventoryEmptyState = (StackPanel)target;
			break;
		case 19:
			inventoryEmptyLabel = (TextBlock)target;
			break;
		case 20:
			assetGrid = (DataGrid)target;
			assetGrid.SelectionChanged += AssetGrid_SelectionChanged;
			break;
		case 21:
			assetEmptyState = (StackPanel)target;
			break;
		case 22:
			assetEmptyLabel = (TextBlock)target;
			break;
		case 23:
		...
		case 25:
			...
			break;
		case 26:
			btnEditSelected = (Button)target;
			btnEditSelected.Click += BtnEditSelected_Click;
			break;
		case 27:
			btnViewSelected = (Button)target;
			btnViewSelected.Click += BtnViewSelected_Click;
			break;
		case 28:
		...
		case 29:
			...
			break;
		case 30:
			footerHintLabel = (TextBlock)target;
			break;
		default:
			_contentLoaded = true;
			break;
