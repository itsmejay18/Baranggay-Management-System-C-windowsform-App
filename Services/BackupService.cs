...
internal static class BackupService
{
    ...
    internal static BackupRunInfo? TryGetLatestRun()
    {
        try
        {
            ...
            return new BackupRunInfo(id, startedAt, endedAt, ParseState(status), filePath, fileSize, error);
        }
        catch (Exception ex)
        {
    ...
    internal static BackupRunInfo RunBackupNow(int? triggeredByUserId, bool compressToZip = true)
    {
        ...
        try
        {
            runId = TryInsertRun(startedAt, triggeredByUserId);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            string baseName = $"{csb.Database}-backup-{startedAt:yyyyMMdd-HHmmss}";
            ...
            var result = new BackupRunInfo(runId, startedAt, endedAt, BackupRunState.Success, finalPath, fileSize, null);
        ...
        catch (Exception ex)
        {
            ...
            var result = new BackupRunInfo(runId, startedAt, endedAt, BackupRunState.Failed, null, null, message);
            TryUpdateRun(result);
            return result;
        }
    }
    ...
    private static long? TryInsertRun(DateTime startedAt, int? createdByUserId)
    {
        using var conn = DBConnection.GetConnection();
        conn.Open();
    ...
    private static void RunMySqlDump(string dumpExe, MySqlConnectionStringBuilder csb, string outputSqlPath)
    {
        ...
        {
            // Avoid prompting, even when empty.
            baseArgs.Insert(6, $"--password={csb.Password}");
        }

        // MySQL 8 client compatibility flags (retry without if unsupported).
        var tryArgs = new[]
        {
            baseArgs.Concat(new [] { "--set-gtid-purged=OFF", "--column-statistics=0" }).ToList(),
            baseArgs
        };

        Exception? lastError = null;
        foreach (var args in tryArgs)
        {
            try
            {
                ...
                (int exitCode, string stderr) = RunProcessCaptureStderr(dumpExe, args);
                if (exitCode != 0)
                {
                    throw new InvalidOperationException(
