...
internal sealed class CollectionsModuleForm : Form
{
    ...
    private readonly Label _cashValue = new Label();
    private readonly Label _gcashValue = new Label();
    private readonly Label _bankValue = new Label();
    ...
    private bool HasCollectionAccess => Permissions.IsAdmin
    ...
    public CollectionsModuleForm(Action<int, int> openCertificateById)
    {
    ...
    private void InitializeComponent()
    {
        ...
        BuildSummaryCard();
        root.Controls.Add(_summaryCard, 0, 2);
        ...
        root.Controls.Add(filterCard, 0, 3);
        ...
        root.Controls.Add(pagerPanel, 0, 5);
    ...
    private void BuildSummaryCard()
    {
        ...
        _summaryCard.BackColor = Color.White;
        _summaryCard.Padding = new Padding(12);
        _summaryCard.Margin = new Padding(0, 0, 0, 10);
        UiTheme.StyleSectionCard(_summaryCard, Color.White, enforceBorder: true, padding: new Padding(12));
        ...
        ConfigureSummaryValueLabel(_totalValue);
        ConfigureSummaryValueLabel(_cashValue);
        ConfigureSummaryValueLabel(_gcashValue);
        ConfigureSummaryValueLabel(_bankValue);

        layout.Controls.Add(_totalValue, 0, 0);
        layout.Controls.Add(_cashValue, 1, 0);
        layout.Controls.Add(_gcashValue, 2, 0);
        layout.Controls.Add(_bankValue, 3, 0);

        layout.Controls.Add(CreateSummaryCaption("Total Collected"), 0, 1);
        layout.Controls.Add(CreateSummaryCaption("Cash"), 1, 1);
        layout.Controls.Add(CreateSummaryCaption("GCash"), 2, 1);
        layout.Controls.Add(CreateSummaryCaption("Bank"), 3, 1);

        _summaryCard.Controls.Add(layout);
        UpdateSummaryValues(0m, 0m, 0m, 0m);
    }
    ...
    private async Task UpdateSummaryAsync(CancellationToken cancellationToken)
    {
...
SELECT
    ...
    COALESCE(SUM(CASE WHEN dp.payment_method = 'Cash' THEN dp.amount ELSE 0 END), 0) AS cash_amount,
    COALESCE(SUM(CASE WHEN dp.payment_method = 'GCash' THEN dp.amount ELSE 0 END), 0) AS gcash_amount,
    COALESCE(SUM(CASE WHEN dp.payment_method = 'Bank' THEN dp.amount ELSE 0 END), 0) AS bank_amount
...
{whereClause}";
        ...
        decimal total = ReadDecimal(row, "total_amount");
        decimal cash = ReadDecimal(row, "cash_amount");
        decimal gcash = ReadDecimal(row, "gcash_amount");
        decimal bank = ReadDecimal(row, "bank_amount");
        UpdateSummaryValues(total, cash, gcash, bank);
    }

    private void UpdateSummaryValues(decimal total, decimal cash, decimal gcash, decimal bank)
    {
        _totalValue.Text = total.ToString("N2");
        _cashValue.Text = cash.ToString("N2");
        _gcashValue.Text = gcash.ToString("N2");
        _bankValue.Text = bank.ToString("N2");
    }
    ...
    private void LoadDocumentTypes()
    {
        ...
        {
            AppLogger.LogWarning("Unable to load document types for collections filter.", ex);
        }
    ...
    {
        bool hasAccess = HasCollectionAccess;
    ...
    private async void RefreshList(bool resetPage)
    {
        ...
        {
            ControllerDialogs.Error(ex, "Unable to load collections.", "Collections");
            ShowErrorState("Unable to load collections right now.");
        }
        finally
        {
...
(
    ...
    private void UpdateStatePanel(int rowCount)
    {
        ...
        string title = hasFilters ? "No collections found" : "No collection records yet";
        string message = hasFilters
            ? "Try adjusting the search text, type filter, method filter, or date range."
            : "Collections will appear here once documents are issued and paid.";
    ...
    private async void ExportListAsync()
    {
        if (!Permissions.CanExportCertificates)
        {
            ControllerDialogs.Warning("You do not have permission to export collections.");
            return;
        }
        ...
        try
        {
            ...
            if (raw.Rows.Count == 0)
            {
                ControllerDialogs.Warning("Nothing to export.", "Export Collections");
                return;
            }
            ...
            using var dialog = new SaveFileDialog
            {
                ...
                FileName = $"collections_{DateTime.Now:yyyyMMdd_HHmmss}.csv"
            };
            ...
            ControllerDialogs.Info("Collections export completed.", "Export Collections");
        }
        catch (OperationCanceledException)
        {
        ...
        {
            ControllerDialogs.Error(ex, "Unable to export collections.", "Export Collections");
        }
        finally
        {
            SetLoading(false, string.Empty);
