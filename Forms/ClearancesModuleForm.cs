...
internal sealed class ClearancesModuleForm : Form
{
    ...
    public ClearancesModuleForm(Action<CertificateAction> openCertificates, Action<int, int> openCertificateById)
    {
    ...
    private void InitializeComponent()
    {
        ...
        _searchBox.PlaceholderText = "Search by document no, resident name, purpose, or OR no...";
    ...
    private async void RefreshList(bool resetPage)
    {
        ...
        SetLoading(true, "Loading clearances...");
        ...
        {
            ControllerDialogs.Error(ex, "Unable to load clearances.", "Clearances");
            ShowErrorState("Unable to load clearances right now.");
        }
        finally
        {
...
ORDER BY dr.requested_at DESC, dr.doc_request_id DESC";
    ...
    private QueryContext BuildQueryContext()
    {
        ...
        context.Parameters.Add(("@docCode", "BC"));
        context.Parameters.Add(("@docName", "Barangay Clearance"));
...
(
    ...
    private DataTable BuildDisplayTable(DataTable source)
    {
        ...
        foreach (DataRow row in source.Rows)
        {
            ...
            string purpose = SafeString(row, "purpose");
    ...
    private void ConfigureGridColumns()
    {
        ...
        {
            _grid.Columns["resident_name"].HeaderText = "Resident";
            _grid.Columns["resident_name"].FillWeight = 140;
        }
        if (_grid.Columns.Contains("purpose"))
        {
            _grid.Columns["purpose"].HeaderText = "Purpose";
            _grid.Columns["purpose"].FillWeight = 150;
        }
    ...
    private void UpdateStatePanel(int rowCount)
    {
        ...
        string title = hasFilters ? "No clearances found" : "No clearance requests yet";
        string message = hasFilters
            ? "Try adjusting the search text, status filter, or date range."
            : "Create your first barangay clearance request to get started.";
    ...
    private void ShowNoPermissionState()
    {
        Panel card = UiTheme.CreateStateCard(
            "Access restricted",
            "You do not have permission to view clearance records.",
            IconChar.Lock,
            UiTheme.AccentRed);
        ShowState(card);
    }
    ...
    private void OpenNewRequest()
    {
        if (!Permissions.CanRequestCertificates)
        {
            ControllerDialogs.Warning("You do not have permission to create clearance requests.");
            return;
        }
    ...
    private void ApproveSelected()
    {
        if (!Permissions.CanApproveCertificates)
        {
            ControllerDialogs.Warning("You do not have permission to approve clearances.");
            return;
        }

        if (!TryGetSelectedCertificate(out int residentId, out int certificateId))
        {
            ControllerDialogs.Warning("Select a clearance first.");
            return;
        }
    ...
    private void ReleaseSelected()
    {
        if (!Permissions.CanIssueCertificates)
        {
            ControllerDialogs.Warning("You do not have permission to release clearances.");
            return;
        }
    ...
    private async void ExportListAsync()
    {
        if (!Permissions.CanExportCertificates)
        {
            ControllerDialogs.Warning("You do not have permission to export clearances.");
            return;
        }
        ...
        try
        {
            ...
            if (raw.Rows.Count == 0)
            {
                ControllerDialogs.Warning("Nothing to export.", "Export Clearances");
                return;
            }

            using var dialog = new SaveFileDialog
            {
                Title = "Export Clearances",
                Filter = "CSV Files (*.csv)|*.csv",
                FileName = $"clearances_{DateTime.Now:yyyyMMdd_HHmmss}.csv"
            };
            ...
            sb.AppendLine("Document No,Resident,Purpose,Status,Requested,Approved,Released,OR No,Fee");
            ...
            foreach (DataRow row in display.Rows)
            {
                sb.AppendLine(string.Join(",", new[]
                {
                    ...
                    EscapeCsv(Convert.ToString(row["purpose"]) ?? string.Empty),
            ...
            ControllerDialogs.Info("Clearance export completed.", "Export Clearances");
        }
        catch (OperationCanceledException)
        {
        ...
        {
            ControllerDialogs.Error(ex, "Unable to export clearances.", "Export Clearances");
        }
        finally
        {
            SetLoading(false, string.Empty);
